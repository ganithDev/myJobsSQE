...
<div class="grid_16">
        ...
        <%
            String ucasPoints="",gcseMathsGrade="",gcseEnglishGrade="",classification="";
            ucasPoints=request.getParameter("sectorID");
            if(ucasPoints.isEmpty()){
            %>
          <p>No Results.</p>
        <%
            }else{
            
            List<Person> Lst = new PersonDAOImpl().searchCVMinEducationLevel(ucasPoints, gcseMathsGrade, gcseEnglishGrade);
        if (Lst.size()==0) {
                %>
                <p>No Results.</p>
        <%
